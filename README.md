...
| `Super+Ctrl+Shift+P` | 系统框选截图并编辑标注 / Native region screenshot with annotation |
...
- **截图标注**仅在 Omarchy 提供：先隐藏翻译面板，框选后立即打开系统编辑器 Tensaku，可圈画、画箭头、添加文字；编辑完成按 Enter 复制并关闭编辑窗口，再粘贴到其他应用。框选时 Esc 取消，翻译程序未运行时也可用。截图仅使用临时文件，编辑器退出后自动清理；不保存到图片目录、不提示保存位置，编辑确认前不修改剪贴板。
